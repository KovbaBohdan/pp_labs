...
public class BookingRepository {
    ...
    public Booking getBookingById(long id) {
        return bookingList.stream().filter(booking -> booking.getId() == id).findAny().orElseThrow(() -> new RuntimeException("No such booking"));
    }
    ...
    public Booking updateBooking(Booking updatedBooking) {
        Booking booking = bookingList.stream().filter(b -> b.getId() == updatedBooking.getId()).findAny().orElseThrow(() -> new RuntimeException("No such booking"));
    ...
    public List<Booking> getBookingsByCottageId(long cottageId) {
        return bookingList.stream().filter(booking -> booking.getCottageId() == cottageId).collect(Collectors.toList());
    }

    public List<Booking> getBookingsByUserId(long userId) {
        return bookingList.stream().filter(booking -> booking.getUserId() == userId).collect(Collectors.toList());
    }

    public boolean isCottageBooked(long cottageId, LocalDate startDate, LocalDate endDate) {
        return bookingList.stream().filter(booking -> booking.getCottageId() == cottageId).anyMatch(booking -> (startDate.isBefore(booking.getEndDate()) && endDate.isAfter(booking.getStartDate())));
    }
    ...
    public List<Booking> getBookingsByHotelId(long hotelId) {
        return bookingList.stream().filter(booking -> booking.getCottage().getHotel().getId() == hotelId).collect(Collectors.toList());
    }
